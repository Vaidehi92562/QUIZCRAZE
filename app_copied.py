...
def multi_quiz_room():
    ...
    # Fetch category from multigames table
    ...
    category = game[0]  # Fetch category from the tuple

    # Fetch 10 random questions for the chosen category
    ...
    # Convert tuple list into list of dictionaries
    questions_list = []
    for q in questions:
...
def submit_quiz():
    ...
    print(f"✅ Time Taken Received: {time_taken} seconds")
 
    if not user_answers:
        print("⚠️ No answers received!")
        return jsonify({"error": "No answers provided!"}), 400

    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)

    # ✅ Fetch correct answers from MySQL
    ...
    for qid, user_ans in user_answers.items():
        ...
        print(f"   → User Answer (Converted): {user_ans_converted} | Type: {type(user_ans_converted)}")
        print(f"   → Correct Answer (DB): {correct_ans} | Type: {type(correct_ans)}")

        if correct_ans and user_ans_converted == correct_ans:
            print(f"✅ Match Found for QID {qid} → +1 Point")
            score += 1
        else:
            print(f"❌ No Match for QID {qid} → +0 Points")

    print(f"✅ Final Score for {username} = {score}")
...
def leaderboard():
    ...
    # Fetch all players for the game, sorted by score (DESC), then by time taken (ASC)
    cursor.execute("""
