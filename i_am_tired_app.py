...
def quiz():
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT id, question, correct_option FROM questions LIMIT 10")
    questions = cursor.fetchall()  # Fetch all questions
    cursor.close()
...
def submit_quiz():
    ...
    raw_time_taken = data.get("timeTaken", "0:00")

    # Convert 'mm:ss' format to total seconds
    try:
        minutes, seconds = map(int, raw_time_taken.split(":"))
        time_taken = minutes * 60 + seconds
    except ValueError:
        time_taken = 0  # Default to 0 if format is incorrect

    print(f"⏳ Converted Time Taken: {raw_time_taken} → {time_taken} seconds")
    ...
    cursor.execute("""
        ...
        FROM questions q
        JOIN multipayersname m ON q.id = m.id
        WHERE m.game_code = %s
    """, (game_code,))
    
    correct_answers = {str(row["id"]): str(row["correct_option"]) for row in cursor.fetchall()}

    print("✅ Received Answers from User:", user_answers)
    print("✅ Correct Answers from DB:", correct_answers)

    # Validate and calculate score
    score = 0
    for question_id, user_answer in user_answers.items():
        if question_id == "undefined" or question_id not in correct_answers:
            print(f"⚠️ ERROR: Received 'undefined' or invalid question ID: {question_id}")
            continue  # Skip this answer

        correct_answer = correct_answers.get(question_id)

        if correct_answer and user_answer.strip() == correct_answer.strip():
            score += 1  # Increase score if correct

    print(f"🏆 Final Calculated Score for {username} = {score}")

    # Store score in MySQL
    cursor.execute("""
        UPDATE multipayersname
    ...
    print(f"✅ Successfully updated score = {score} and time = {time_taken} for {username}")
...
def leaderboard():
    ...
    # ✅ Ensure correct table is queried for username, score, and time_taken
    cursor.execute("""
    ...
    if not players:
        return jsonify({"error": "No players found for this game code"}), 404
